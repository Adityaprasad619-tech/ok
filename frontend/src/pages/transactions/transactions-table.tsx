...
import {setRefetch, uploadCsv} from '../../stores/transactions/transactionsSlice';
...
const TransactionsTablesPage = () => {
  ...
  const [filters] = useState([
          ...
          {label: 'AmountinSHM', title: 'amount', number: 'true'},
          {label: 'TransactionDate', title: 'transaction_date', date: 'true'},

    {label: 'TransactionType', title: 'transaction_type', type: 'enum', options: ['Send','Receive','Tip','Subscription','Crowdfunding','Rent','CouponPurchase']},
  ]);
    const addFilter = () => {
        const newItem = {
            id: uniqueId(),
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title="Transactions" main>
        {''}
        ...
        <CardBox className='mb-6' cardBoxClassName='flex flex-wrap'>
          ...
          <BaseButton
              ...
              onClick={addFilter}
          />
          <BaseButton className={'mr-3'} color='info' label='Download CSV' onClick={getTransactionsCSV} />
            <BaseButton
              color='info'
              label='Upload CSV'
